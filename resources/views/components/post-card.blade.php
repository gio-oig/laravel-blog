<div class="col-md-4">
    <div class="card mb-4 box-shadow">
        ...
        <div class="card-body">
            ...
            <p class="card-text">{{ Str::substr($post->text, 0, 150) }}...</p>
            <div class="d-flex justify-content-between align-items-center">
                <div class="btn-group">
                    <a href="{{ route('front.post', ['slug' => $post->slug]) }}"
                        class="btn btn-sm btn-outline-secondary">View</a>

                    <a href="{{ route('front.category', ['slug' => $post->category_slug ?? 'defaultslug']) }}"
                        class="btn btn-sm btn-outline-secondary">{{ $post->category_name ?? $post->category->name }}</a>
                </div>
